...
class PermissionsTable extends AclNodesTable
{
    ...
    public function check($aro, $aco, $action = '*')
    {
        ...
        //frontend route check off start @22/3/2023 akash
        // echo $aco;die;
        $new_aco  = explode("/", $aco);
        $count_ = count($new_aco);
        $where['controller'] = $new_aco[$count_ - 2];
        $where['action'] = $new_aco[$count_ - 1];
         
        $frontend_route = TableRegistry::getTableLocator()->get('scms_frontend_route');
        $frontend_routes = $frontend_route->find()->where($where)->toArray();
        if (count($frontend_routes) > 0) {
            return true;
        }
        //frontend route check off end @22/3/2023 akash
